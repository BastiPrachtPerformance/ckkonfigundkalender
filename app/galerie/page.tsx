...
export default function GalleryPage() {
  return <main className="gallery-page" id="top">
    ...
    <section className="gallery-masonry">
      {gallery.map(([src, alt], index) => <figure className={`masonry-item item-${index + 1}`} key={src}><img src={src} alt={alt} loading={index > 3 ? "lazy" : "eager"} /><figcaption><span>{String(index + 1).padStart(2, "0")}</span>{index % 3 === 0 ? "CK Eventcenter" : index % 3 === 1 ? "CK Garden" : "Momente"}</figcaption></figure>)}
    </section>
